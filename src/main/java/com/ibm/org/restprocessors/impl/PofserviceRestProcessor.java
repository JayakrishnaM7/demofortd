...
@Service
public class PofserviceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PofserviceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.pofservice.hosturl}")
	private String url;
	
	@Value("${spring.api.pofservice.context-path}")
	private String contextPath;

	@Value("${spring.api.pofservice.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PofserviceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PofserviceRestProcessor.class.getName());
