...
@Service
public class EbranchopsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(EbranchopsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.ebranchops.hosturl}")
	private String url;
	
	@Value("${spring.api.ebranchops.context-path}")
	private String contextPath;

	@Value("${spring.api.ebranchops.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  EbranchopsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ EbranchopsRestProcessor.class.getName());
