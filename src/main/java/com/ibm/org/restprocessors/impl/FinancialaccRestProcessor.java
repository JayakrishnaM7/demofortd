...
@Service
public class FinancialaccRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(FinancialaccRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.financialacc.hosturl}")
	private String url;
	
	@Value("${spring.api.financialacc.context-path}")
	private String contextPath;

	@Value("${spring.api.financialacc.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  FinancialaccRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ FinancialaccRestProcessor.class.getName());
