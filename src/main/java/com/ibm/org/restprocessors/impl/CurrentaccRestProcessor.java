...
@Service
public class CurrentaccRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CurrentaccRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.currentacc.hosturl}")
	private String url;
	
	@Value("${spring.api.currentacc.context-path}")
	private String contextPath;

	@Value("${spring.api.currentacc.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CurrentaccRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CurrentaccRestProcessor.class.getName());
