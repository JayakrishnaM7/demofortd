...
@Service
public class TxauthorisationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(TxauthorisationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.txauthorisation.hosturl}")
	private String url;
	
	@Value("${spring.api.txauthorisation.context-path}")
	private String contextPath;

	@Value("${spring.api.txauthorisation.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  TxauthorisationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ TxauthorisationRestProcessor.class.getName());
