...
@Service
public class CustomerprodsandservicesRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CustomerprodsandservicesRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customerprodsandservices.hosturl}")
	private String url;
	
	@Value("${spring.api.customerprodsandservices.context-path}")
	private String contextPath;

	@Value("${spring.api.customerprodsandservices.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CustomerprodsandservicesRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CustomerprodsandservicesRestProcessor.class.getName());
