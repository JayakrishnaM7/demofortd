...
@Service
public class CworkbenchRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CworkbenchRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.cworkbench.hosturl}")
	private String url;
	
	@Value("${spring.api.cworkbench.context-path}")
	private String contextPath;

	@Value("${spring.api.cworkbench.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CworkbenchRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CworkbenchRestProcessor.class.getName());
