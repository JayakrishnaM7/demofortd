...
@Component
public class Pointofservice implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private PofserviceRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Pointofservice.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Pointofservice.class.getName());
         }else{
