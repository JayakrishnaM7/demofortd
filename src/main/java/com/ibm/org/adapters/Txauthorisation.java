...
@Component
public class Txauthorisation implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private TxauthorisationRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Txauthorisation.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Txauthorisation.class.getName());
         }else{
